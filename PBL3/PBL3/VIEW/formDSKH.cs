...
namespace PBL3
{
    public partial class formDSKH : Form
    {
        ...
        private void showKH()
        {
            dataGridView.DataSource = BLL_KhachHang.Instance.getAllKhachHang(txtSearch.Text,cbbSort.Text);
        ...
        {
            KhachHang khachHang = BLL_KhachHang.Instance.findKhachHangById(id);
            if (khachHang != null)
            {
        ...
        private void addOrUpdate(string id)
        {
            ...
            txtQuocTich.Text = khachHang.QuocTich = txtQuocTich.Text;
            ...
            BLL_KhachHang.Instance.addOrUpdate(khachHang);           
            showKH();
        }

        private void btnAdd_Click(object sender, EventArgs e)
        {
            if (BLL_KhachHang.Instance.findKhachHangById(txtMaKH.Text)==null)
            {
                try
                {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (dataGridView.SelectedRows.Count > 0)
            {
                foreach (DataGridViewRow i in dataGridView.SelectedRows)
                {
                    ...
                    {
                        BLL_KhachHang.Instance.delete(i.Cells["IdKhachHang"].Value.ToString());
                    }
                    else
                    {
